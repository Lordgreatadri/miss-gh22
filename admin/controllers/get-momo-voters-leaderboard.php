<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        $query = "SELECT DISTINCT `momo_number` as voter_num, SUM(vote_count) as vote_num FROM `momo_completed_transactions` WHERE response_code = '0000' AND vote_channel = 'momo'  GROUP BY `momo_number` ORDER BY vote_num DESC LIMIT 5";//OR `channel` = 'mtn-gh' OR `channel` = 'vodafone-gh' OR `channel` = 'airtel-gh' OR `channel` = 'tigo-gh'
