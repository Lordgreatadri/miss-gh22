<?php
  ...
  include_once "db-config.php";
      ...
      $filename = "../data_exports/".date('Y').'-'.strtotime("").'missgh_votes_'.date('d-M').'.csv';
    ...
    $insertUserQuery = "UPDATE log_hist SET raw_data = 'Raw vote data exported' WHERE session_id = '$session_id' AND username = '$username'";
    mysqli_query($database, $insertUserQuery);



      
          #*************         SQL SELECT STATEMENT        ********************************* contestant_id AS CODE,
    $sql = "SELECT  contestant_name AS NAME, status AS STATUS,  num_of_votes AS VOTES, updated_at AS 'LAST VOTED' FROM contestants WHERE status = 'not_evicted' ORDER BY num_of_votes DESC";
